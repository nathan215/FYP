...
def interpolate_location(drone_before, drone_after, station_time_str):
    ...
    interpolated_height = drone_before.get('height', 0) + (drone_after.get('height', 0) - drone_before.get('height', 0)) * ratio
...
def process_combined_data(new_station_point,websocket_server):
    for i in range(len(drone_data) - 1):
        if drone_data[i]['time'] <= new_station_point['time'] <= drone_data[i+1]['time']:
            ...
            websocket_server.send_data(combined_data)
...
def start_combining_data(websocket_server):
    ...
    thread = threading.Thread(target=start_combining_data_once, args=(websocket_server,))
    thread.start()